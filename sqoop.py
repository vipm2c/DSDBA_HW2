...
def main():
    opts, args = getopt.getopt(sys.argv[1:], "s:l:p:", ["server=", "login=", "password="])
    count = 0
    server = ""
    login = ""
    password = ""
    ...
    shutil.rmtree('input')
    os.system("/opt/sqoop/bin/sqoop import -fs local --connect jdbc:postgresql://"+server+":5432/hw2?ssl=false --username "+login+" --password "+password+" --query \"SELECT * FROM public.data WHERE \$CONDITIONS ORDER BY (timestamp, id)\" --target-dir input -m 1")#--table data --target-dir input -m 1")
    print("=====Scoop Done=====")
